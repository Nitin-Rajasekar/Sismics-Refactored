...
public class ImportResource extends BaseResource {
    ...
    public Response list() {
        ...
        for (ImportAudioFile importedFile : importedFileList) {
            ...
            item.add("user_filter", importedFile.getUser_filter());
            items.add(item);
        }
    ...
    public Response tag(
        ...
        // File filei = new File("output.txt");
        ...
        // Tag the file
        try {
            AppContext.getInstance().getImportAudioService().tagFile(fileName, order, title, album, artist, albumArtist, directory, user_filter);
        } catch (Exception e) {
