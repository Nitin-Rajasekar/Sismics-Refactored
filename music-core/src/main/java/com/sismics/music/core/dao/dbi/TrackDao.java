...
public class TrackDao extends BaseDao<TrackDto, TrackCriteria> {
    ...
    protected QueryParam getQueryParam(TrackCriteria criteria, FilterCriteria filterCriteria) {
        ...
        StringBuilder sb = new StringBuilder("select t.id as id, t.filename as fileName, t.title as title, t.year as year, t.genre as genre, t.length as length, t.bitrate as bitrate, t.number as trackOrder, t.vbr as vbr, t.format as format, t.user_filter as user_filter, ");
    ...
    public String create(Track track) {
        ...
        handle.createStatement("insert into " +
                "  t_track(id, album_id, artist_id, filename, title, titlecorrected, year, genre, length, bitrate, number, vbr, format, createdate, user_filter)" +
                "  values(:id, :albumId, :artistId, :fileName, :title, :titleCorrected, :year, :genre, :length, :bitrate, :number, :vbr, :format, :createDate, :user_filter)")
                .bind("id", track.getId())
